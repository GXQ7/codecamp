...
# List
- an ordered collection / sequence of objects 
- control over insertion of list by index
...
```
## ArrayList
- dynamic array which can grow and shrink (don't need to know size at creation)
- more functionality due to implementing List interface
- allows null
- dynamic increase of capacity by 50%
- unsynchronised 
- e.g. storing youtube subscriptions
> Time Complexity is the same as array

```Java
        String[] fruits = new String[1];
        fruits[0] = "strawberry";
        //fruits[1] = "apple";     //IndexOutOfBounds
 
        ArrayList<String> fruitss = new ArrayList<String>(1);
        fruitss.add("strawberry");
        fruitss.add("apple");
        fruitss.add("banana");

        String fruit = "raspberry";
        var checkForFruit = (fruitss.contains(fruit)) ? fruitss.remove(fruit) : fruitss.add(fruit);
```
### Vector 
- similar to ArrayList
- thread safe = slow
- dynamically increase of capacity by 100%
...
```
### Priority Queue
- abstract data type - can be imlpemented with several data structures
