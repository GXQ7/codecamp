...
- root, sub-trees, children, leaves
- applications: file system on a computer, easy searching, data manipulation, router algorithms
- pros: tree's with ordering (BST) provide moderate access/search & moderate insertion/ deletion compared to LL & array
- cons:

## Binary Tree
- root has at least 2 children [left_node] (smaller than parent) & [right_node] (greater than parent)
- operations on average are O(log n), worst case O(n) if the tree is not balanced, creating a linked list
