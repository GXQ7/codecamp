...
- root, sub-trees, parents, children, siblings leaves
...
- pros: operations take O(h) time where h is the the height of the tree
- cons: if a sorted list is inserted into the tree operations are now O(n) as opposed to O(log (n))

### Properties (Hand Shaking Lemma)
